...
intents.members  = True   # enable "Server Members Intent" in Discord Dev Portal too
...
async def on_ready():
    ...
    if GUILD_ID:
        # Instant sync to one specific server (shows commands immediately)
...
async def cmd_autofill(interaction: discord.Interaction):
    ...
    # Try to look up Discord roles — works only if Members Intent is enabled
    try:
        ...
        player_roles = {p: set() for p in data["players"]}
